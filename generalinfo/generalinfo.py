...
class GeneralInfo(commands.Cog, name=__plugin_name__):
    ...
    async def botinfo(self, ctx: commands.Context):
        """
        Information of the TDL Modmail bot.
        """
        bot_me = ctx.me
    ...
    async def useravatar(self, ctx: commands.Context, *, member: discord.Member = None):
        """
        Avatar (profile picture) of a TDL member.
    ...
    async def userinfo(self, ctx: commands.Context, *, user: Union[discord.Member, discord.User] = None):
        """
        Information of a TDL server member or Discord user.
    ...
    async def roleinfo(self, ctx: commands.Context, *, role: discord.Role):
        """
        Information of a specified role in the TDL Discord server.
    ...
    async def rolemembers(self, ctx: commands.Context, *, role: discord.Role):
        """
        List of TDL members in a role specified.
        ...
        if member_list:
            ...
            embeds[0].description = "Currently there are no members with that role here at TDL"
    ...
    async def allroles(self, ctx: commands.Context):
        """
        List of roles on the TDL discord server.
        """
        roles_list = [
        ...
        def base_embed(continued=False, description=None):
            embed = discord.Embed(title="All roles in the TDL Discord Server ", color=self.bot.main_color)
            if continued:
                embed.title += " (Continued)"
        ...
        if roles_list:
            ...
            embeds[0].description = "There is no role in the TDL server."
    ...
    async def serverinfo(self, ctx: commands.Context):
        """
        Information of the TDL Discord server.
        """
        guild = ctx.guild
        ...
        if guild_features_list:
            embed.add_field(name="__TDL Server features:__", value="\n".join(guild_features_list))

        embed.add_field(name="TDL Owner:__", value=guild.owner.mention, inline=False)
    ...
    async def guildemojis(self, ctx: commands.Context):
        """
        List of custom emojis in the TDL Discord server with their IDs.
        """
        pages = []
        page = "### Server emojis:\n\n"
